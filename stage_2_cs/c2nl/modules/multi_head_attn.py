...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
        ...
        def shape(x, dim):
            """projection"""
            return x.view(batch_size, -1, head_count, dim).transpose(
                1, 2
            )
        ...
        if layer_cache is not None:
            if attn_type == "self":
                ...
                if layer_cache["self_keys"] is not None:
                    key = torch.cat(
                        (layer_cache["self_keys"], key), dim=2
                    )
        ...
        if self.max_relative_positions > 0 and attn_type == "self":
            scores = query_key + relative_matmul(
                query, relations_keys, True
            )
        else:
            scores = query_key
        scores = scores.float()
        ...
        if self._coverage and attn_type == "context":
            ...
            if (
                step is not None
            ):  # indicates inference mode (one-step at a time)
                if coverage is None:
                    ...
                    assert (
                        coverage.dim() == 4
                    )  # B x num_heads x 1 x key_len
                    unnormalized_score = exp_score.div(coverage + 1e-20)
            else:
                multiplier = torch.tril(
                    torch.ones(query_len - 1, query_len - 1)
                )
        ...
        attn_per_head = [
            attn.squeeze(1) for attn in attn.chunk(head_count, dim=1)
        ]

        covrage_vector = None
        if (
            self._coverage and attn_type == "context"
...
class MultiHeadedAttention_arpe(nn.Module):
    """
    Multi-Head Attention module from
    "Attention is All You Need"
    :cite:`DBLP:journals/corr/VaswaniSPUJGKP17`.
    ...
    def __init__(
        ...
        super(MultiHeadedAttention_arpe, self).__init__()
        ...
        if max_relative_positions > 0:
            ...
            self.ast_positions_embeddings_k = nn.Embedding(
                97, self.d_k
            )  # java 97  python 32
            self.relative_positions_embeddings_v = nn.Embedding(
                vocab_size, self.d_v
            )
            self.ast_positions_embeddings_v = nn.Embedding(97, self.d_v)
    ...
    def forward(
        ...
        if layer_cache is not None:
            ...
            key = shape(
                self.key(key), self.d_k
            )  # [B, key_len, 512] -> [[B, 8, key_len, 64]]
            value = shape(self.value(value), self.d_v)
            query = shape(self.query(query), self.d_k)

        if (
            self.max_relative_positions > 0 and attn_type == "self"
        ):  # 64
            key_len = key.size(2)
            # 1 or key_len x key_len
            relative_positions_matrix = (
            ...
            #  1 or key_len x key_len x dim_per_head    dim_per_head = 64, dim = 8 * 64 = 512
            relations_keys = (
                self.relative_positions_embeddings_k(  # [34, 34, 64]
                    relative_positions_matrix.to(key.device)
                )
            )
            ast_rpe = torch.clamp(ast_rpe, min=0, max=64)
            ast_keys = self.ast_positions_embeddings_k(ast_rpe)
            #  1 or key_len x key_len x dim_per_head
            relations_values = self.relative_positions_embeddings_v(
                relative_positions_matrix.to(key.device)
            )
            ast_values = self.ast_positions_embeddings_v(ast_rpe)
        ...
        if self.max_relative_positions > 0 and attn_type == "self":
            ...
            scores = (
                query_key
                + relative_matmul(query, relations_keys, True)
                + relative_matmul_ast(query, ast_keys, True)
            )
        else:
            scores = query_key
        scores = scores.float()
        ...
        if self.max_relative_positions > 0 and attn_type == "self":
            context = unshape(
                context_original
                + relative_matmul(drop_attn, relations_values, False)
                + relative_matmul_ast(drop_attn, ast_values, False),
                self.d_v,
            )
        else:
            context = unshape(context_original, self.d_v)
