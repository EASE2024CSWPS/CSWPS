    ...
    * Bahdanau Attention (mlp):
    ...
    def score(self, h_t, h_s):
        ...
        if self.attn_type in ["general", "dot"]:
            ...
            return self.v(wquh.view(-1, dim)).view(
    ...
    def forward(
        ...
        if self._coverage:
            ...
            if one_step:
                ...
                multiplier = torch.tril(
                    torch.ones(target_l - 1, target_l - 1)
                )
                ...
                no_penalty = torch.ones_like(
                    penalty[:, -1, :]
                )  # B x slen
                penalty = torch.cat(
                    [no_penalty.unsqueeze(1), penalty], dim=1
                )  # B x tlen x slen
        ...
        # concatenate
        concat_c = torch.cat([c, source], 2).view(
            batch * target_l, dim * 2
        )
