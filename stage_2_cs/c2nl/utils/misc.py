...
def validate(sequence):
    seq_wo_punc = sequence.translate(
...
def generate_relative_positions_matrix(
    length, max_relative_positions, use_neg_dist, cache=False
):
    ...
    if cache:
        ...
        range_mat = (
            range_vec.unsqueeze(-1).expand(-1, length).transpose(0, 1)
        )
...
def relative_matmul_ast(x, z, transpose):
    """Helper function for relative positions attention."""
    x_t = x.permute(2, 0, 1, 3)
    z_p = z.permute(1, 0, 2, 3)
    if transpose:
        z_t = z_p.transpose(2, 3)
        x_t_matmul = torch.matmul(x_t, z_t)
    else:
        x_t_matmul = torch.matmul(x_t, z_p)
    result = x_t_matmul.permute(1, 2, 0, 3)
    return result
...
def count_files(dir_path):
    ...
    """
    # print(dir_path)
    num = subprocess.check_output(f"ls {dir_path}|wc -l", shell=True)
    num = num.decode("utf-8").split(" ")
    return int(num[0])
