...
from c2nl.modules.multi_head_attn import (
    MultiHeadedAttention,
    MultiHeadedAttention_arpe,
)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if sytax_aware:
            ...
            self.attention_arpe = MultiHeadedAttention_arpe(
                heads,
                d_model,
                d_k,
                d_v,
    ...
    def forward(self, inputs, mask, ast_rpe=None, ast_distance=None):
        ...
        if ast_distance is not None:
            ...
            context, attn_per_head, _ = self.attention(
                H_final, H_final, H_final, mask=mask, attn_type="self"
            )
            out = self.layer_norm(self.dropout(context) + H_final)
        else:
            context, attn_per_head, _ = self.attention_arpe(
                inputs,
                inputs,
                inputs,
                mask=mask,
...
class TransformerEncoder(EncoderBase):
    ...
    def count_parameters(self):
        params = list(self.v_layer.parameters()) + list(
            self.s_layer.parameters()
        )
    ...
    def forward(
        ...
        mask = (
            None
            if lengths is None
            else ~sequence_mask(lengths, out.shape[1]).unsqueeze(1)
        )
        if adjacency is not None:
            adjacency = ~(
                adjacency[:, : mask.shape[2], : mask.shape[2]].bool()
                * ~mask
            )
        ...
        for i in range(self.num_layers // 2):
            out, attn_per_head = self.v_layer[i](
                out, mask, ast_rpe, ast_distance
            )
            representations.append(out)
            attention_scores.append(attn_per_head)

            out_, attn_per_head = self.s_layer[i](
                out, adjacency, ast_rpe
            )
            out = out + out_
            representations.append(out)
            attention_scores.append(attn_per_head)
