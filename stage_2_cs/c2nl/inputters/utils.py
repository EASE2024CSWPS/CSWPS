...
def process_examples(
    ...
    code_tokens = [repo] + code_tokens[: max_src_len - 1]
...
def load_data(
    ...
    with open(filenames["guid"]) as f:
        guids = [
            line.strip()
            for line in tqdm(
                f, total=count_file_lines(filenames["guid"])
            )
        ]
    ...
    examples = []
    for repo, guid, src, src_tag, tgt in tqdm(
        zip(repos, guids, sources, source_tags, targets),
        total=len(sources),
    ):
        if dataset_name in ["java", "python", "PCS", "CodeXGLUE"]:
...
def build_word_and_char_dict(
    ...
    dictionary = UnicodeCharsVocabulary(
