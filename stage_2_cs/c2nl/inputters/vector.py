import numpy as np
import torch
from c2nl.utils.misc import count_files

# def sample_k(n, k, test=False):
#     if test:
#         random.seed(1024)
#     else:
#         random.seed()
#     """Generate a random sample of k elements from [0, n)."""
#     if k < n:
...
def vectorize(ex, model):
    ...
    vectorized_ex["code_word_rep"] = torch.LongTensor(
        code.vectorize(word_dict=src_dict)
    )
    if model.args.use_src_char:
        vectorized_ex["code_char_rep"] = torch.LongTensor(
            code.vectorize(word_dict=src_dict, _type="char")
        )
    ...
    if code.language == 3:
        language = "PCS"
    elif code.language == 4:
        language = "CodeXGLUE"
    else:
        raise NotImplementedError(
            f"language id {code.language} not supported"
        )

    repo_cnt = count_files(f"{model.args.repos}/{code.repo}/")

    length = len(code.tokens)  #
    if length > 512:
        length = 512
    guid = code.guid
    struc = np.load(
        "../../data/{}/structure/{}.npy".format(language, guid)
    )
    y = np.load("../../data/{}/distance/{}.npy".format(language, guid))
    arpe = y.copy()
    y = y.astype(np.float16)
    if length > 1:
        y = np.reciprocal(
            (y[:length, :length] + 0.01).astype(np.float16)
        )
        row, col = np.diag_indices_from(y)
        y[row, col] = 0
        ast = y / y.sum(axis=1, keepdims=True)
        if length < 512:
            ast = np.pad(ast, (0, 512 - length), "constant")
    else:
        ast = y

    vectorized_ex["repo_cnt"] = repo_cnt
    ...
    if summary is not None:
        ...
        vectorized_ex["summ_word_rep"] = torch.LongTensor(
            summary.vectorize(word_dict=tgt_dict)
        )
        if model.args.use_tgt_char:
            vectorized_ex["summ_char_rep"] = torch.LongTensor(
                summary.vectorize(word_dict=tgt_dict, _type="char")
            )
        # target is only used to compute loss during training
        vectorized_ex["target"] = torch.LongTensor(
            summary.vectorize(tgt_dict)
        )
    ...
    vectorized_ex["repo"] = code.repo
    vectorized_ex["struc"] = torch.LongTensor(struc)
    vectorized_ex["arpe"] = torch.LongTensor(arpe)
    vectorized_ex["ast"] = torch.LongTensor(ast)
...
def batchify(batch):
    ...
    code_struc = [ex["struc"] for ex in batch]
    ast_distance = [ex["ast"] for ex in batch]
    ast_rpe = [ex["arpe"] for ex in batch]

    max_code_len = max([d.size(0) for d in code_words])
    if use_src_char:
        max_char_in_code_token = code_chars[0].size(1)

    # repo_len_rep = torch.zeros(batch_size, dtype=torch.long)

    # Batch Code Representations
    code_len_rep = torch.zeros(batch_size, dtype=torch.long)
    code_word_rep = (
        torch.zeros(batch_size, max_code_len, dtype=torch.long)
        if use_src_word
        else None
    )
    code_type_rep = (
    ...
    code_struc_rep = torch.zeros(
        batch_size, max_code_len, max_code_len, dtype=torch.long
    )
    ast_distance_rep = torch.zeros(
        batch_size, max_code_len, max_code_len, dtype=torch.float
    )
    ast_rpe_rep = torch.zeros(
        batch_size, max_code_len, max_code_len, dtype=torch.long
    )
    ...
    for i in range(batch_size):
        ...
        # repo_len_rep[i] = class_repo[i].size(0)
        if use_src_word:
            code_word_rep[i, : code_words[i].size(0)].copy_(
                code_words[i]
            )
        if use_code_type:
        ...
        if use_src_char:
            code_char_rep[i, : code_chars[i].size(0), :].copy_(
                code_chars[i]
            )

        code_struc_rep[i, :, :].copy_(
            code_struc[i][:max_code_len, :max_code_len]
        )
        ast_distance_rep[i, :, :].copy_(
            ast_distance[i][:max_code_len, :max_code_len]
        )
        ast_rpe_rep[i, :, :].copy_(
            ast_rpe[i][:max_code_len, :max_code_len]
        )
    ...
    if no_summary:
        ...
        summ_word_rep = (
            torch.zeros(batch_size, max_sum_len, dtype=torch.long)
            if use_tgt_word
            else None
        )
        summ_char_rep = (
        ...
        tgt_tensor = torch.zeros(
            batch_size, max_tgt_length, dtype=torch.long
        )
        alignments = []
        for i in range(batch_size):
            summ_len_rep[i] = summ_words[i].size(0)
            if use_tgt_word:
                summ_word_rep[i, : summ_words[i].size(0)].copy_(
                    summ_words[i]
                )
            if use_tgt_char:
                summ_char_rep[i, : summ_chars[i].size(0), :].copy_(
                    summ_chars[i]
                )
            #
            ...
            align_mask = torch.LongTensor(
                [src_vocabs[i][w] for w in target]
            )
    ...
    return {
        ...
        "repo_cnt": torch.LongTensor([ex["repo_cnt"] for ex in batch]),
        "repos": [ex["repo"] for ex in batch],
        "stype": [ex["stype"] for ex in batch],
        "code_struc_rep": code_struc_rep,
        "ast_distance": ast_distance_rep,
        "ast_rpe": ast_rpe_rep,
    }
