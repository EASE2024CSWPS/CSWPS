...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        ori_query = query_norm

        mid, attn, coverage = self.context_attn(
            memory_bank,
            memory_bank,
            query_norm,
            mask=src_pad_mask,
        ...
        query_norm = self.layer_norm_2(ori_query + self.drop(mid))
...
class TransformerDecoder(DecoderBase):
    ...
    def forward(
        ...
        src_pad_mask = ~sequence_mask(
            state["src_len"], max_len=state["src_max_len"]
        ).unsqueeze(1)
        ...
        for i, layer in enumerate(self.layer):
            ...
            output, attn, coverage = layer(
                ...
                step=step,
                coverage=None
                if layer_wise_coverage is None
