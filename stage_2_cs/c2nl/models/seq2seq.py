...
class Encoder(nn.Module):
    ...
    def forward(self, input, input_len):
        ...
        if self.use_all_enc_layers:
            output = torch.stack(
                layer_outputs, dim=2
            )  # batch_size x seq_len x nlayers x nhid
...
class Decoder(nn.Module):
    def __init__(self, args, input_size):
        ...
        if self.split_decoder:
            ...
            self.fusion_sigmoid = nn.Sequential(
                nn.Linear(args.nhid * 2, args.nhid), nn.Sigmoid()
            )
            self.fusion_gate = nn.Sequential(
                nn.Linear(args.nhid * 2, args.nhid), nn.ReLU()
            )
    ...
    def init_decoder(self, hidden):
        if self.split_decoder:
            return self.copier.init_decoder_state(
                hidden
            ), self.decoder.init_decoder_state(hidden)
        else:
            return self.decoder.init_decoder_state(hidden)

    def decode(self, tgt, dec_state, memory_bank, mem_len):
        if self.split_decoder:
            copier_out, _, attns = self.copier(
                tgt, memory_bank, dec_state[0], memory_lengths=mem_len
            )
            dec_out, _, _ = self.decoder(
                tgt, memory_bank, dec_state[1], memory_lengths=mem_len
            )
            f_t = self.fusion_sigmoid(
                torch.cat([copier_out, dec_out], dim=-1)
            )
            ...
            decoder_outputs, _, attns = self.decoder(
                tgt, memory_bank, dec_state, memory_lengths=mem_len
            )

        decoder_outputs = self.dropout(decoder_outputs)
        return decoder_outputs, attns

    def forward(self, hidden, memory_bank, memory_len, tgt_emb):
        init_decoder_state = self.init_decoder(hidden)
        if self.conditional_decoding:
            encoder_final = init_decoder_state.hidden[
                0
            ]  # layers x batch x nhid
            encoder_final = (
                encoder_final[-1]
                .unsqueeze(1)
        ...
        return self.decode(
            tgt_emb, init_decoder_state, memory_bank, memory_len
        )
...
class Seq2seq(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        if self.copy_attn:
            self.copy_generator = CopyGenerator(
                args.nhid, tgt_dict, self.generator
            )
    ...
    def _run_forward_ml(
        ...
        **kwargs
        ...
        decoder_outputs, attns = self.decoder(
            hidden, memory_bank, code_len, summ_emb
        )
    ...
    def __generate_sequence(
        self, init_decoder_state, params, choice="greedy"
    ):
        ...
        for idx in range(params["max_len"] + 1):
            tgt = self.embedder(tgt_words, tgt_chars, mode="decoder")
            if self.decoder.conditional_decoding:
                tgt = torch.cat([tgt, encoder_final], dim=2)

            decoder_outputs, attns = self.decoder.decode(
                tgt,
                init_decoder_state,
                params["memory_bank"],
                params["src_len"],
    ...
    def decode(
        ...
        **kwargs
        ...
        dec_preds, attentions, copy_info, _ = self.__generate_sequence(
            init_decoder_state, params, choice="greedy"
        )
