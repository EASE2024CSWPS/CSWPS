...
from c2nl.modules.copy_generator import (
    CopyGenerator,
...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        if self.use_tgt_word:
            self.tgt_word_embeddings = Embeddings(
                args.emsize, args.tgt_vocab_size, constants.PAD
            )
        ...
        if self.use_tgt_char:
            assert len(args.filter_size) == len(args.nfilters)
            self.tgt_char_embeddings = CharEmbedding(
                args.n_characters,
                args.char_emsize,
                args.filter_size,
                args.nfilters,
            ...
            self.tgt_highway_net = Highway(
                self.dec_input_size, num_layers=2
            )

        self.use_type = args.use_code_type
        if self.use_type:
            self.type_embeddings = nn.Embedding(
                len(constants.TOKEN_TYPE_MAP), self.enc_input_size
            )
        ...
        if self.tgt_pos_emb:
            self.tgt_pos_embeddings = nn.Embedding(
                args.max_tgt_len + 2, self.dec_input_size
            )
    ...
    def forward(
        ...
        if mode == "encoder":
            ...
            if self.src_pos_emb and self.no_relative_pos:
                pos_enc = torch.arange(
                    start=0, end=word_rep.size(1)
            ...
            word_rep = None
            if self.use_tgt_word:
                word_rep = self.tgt_word_embeddings(
                    sequence.unsqueeze(2)
                )  # B x P x d
            if self.use_tgt_char:
                char_rep = self.tgt_char_embeddings(
                    sequence_char
                )  # B x P x f
                if word_rep is None:
                    word_rep = char_rep
...
class Encoder(nn.Module):
    ...
    def forward(
        self, input, input_len, adjacency, ast_rpe, ast_distance
    ):
        layer_outputs, _ = self.transformer(
            input, input_len, adjacency, ast_rpe, ast_distance
        )  # B x seq_len x h
        if self.use_all_enc_layers:
            output = torch.stack(
                layer_outputs, dim=2
...
class Decoder(nn.Module):
    ...
    def init_decoder(self, src_lens, max_src_len):
        if self.split_decoder:
    ...
    def decode(
        ...
        if self.split_decoder:
            copier_out, attns = self.transformer_c(
                tgt_words,
                tgt_emb,
                memory_bank,
                state[0],
            ...
            dec_out, _ = self.transformer_d(
                tgt_words, tgt_emb, memory_bank, state[1], step=step
            )
            f_t = self.fusion_sigmoid(
                torch.cat([copier_out[-1], dec_out[-1]], dim=-1)
            )
            gate_input = torch.cat(
                [copier_out[-1], torch.mul(f_t, dec_out[-1])], dim=-1
            )
            decoder_outputs = self.fusion_gate(gate_input)
            decoder_outputs = [decoder_outputs]
        else:
            decoder_outputs, attns = self.transformer(
                tgt_words,
                tgt_emb,
                memory_bank,
                state,
    ...
    def forward(self, memory_bank, memory_len, tgt_pad_mask, tgt_emb):
        max_mem_len = (
            memory_bank[0].shape[1]
            if isinstance(memory_bank, list)
            else memory_bank.shape[1]
        )
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        self.generator = nn.Linear(
            self.decoder.input_size, args.tgt_vocab_size
        )
        ...
        if self._copy:
            self.copy_attn = GlobalAttention(
                dim=self.decoder.input_size, attn_type=args.attn_type
            )
            self.copy_generator = CopyGenerator(
                self.decoder.input_size, tgt_dict, self.generator
            )
            self.criterion = CopyGeneratorCriterion(
                vocab_size=len(tgt_dict), force_copy=args.force_copy
            )
        else:
            self.criterion = nn.CrossEntropyLoss(reduction="none")

        self.repo_dis_fc = nn.Linear(self.decoder.input_size, 8000)
        self.repo_k = 20
        self.repo_fc = nn.Linear(
            self.decoder.input_size, self.decoder.input_size
        )
        self.input_proj = nn.Linear(
            self.decoder.input_size, self.decoder.input_size, bias=False
        )
    ...
    def _run_forward_ml(
        ...
        **kwargs,
        ...
        # repo_rep = kwargs['repo_rep']
        repo_len = kwargs["repo_len"]
        repo_dir = kwargs["repo_dir"]
        code_struc_rep = kwargs["code_struc_rep"]
        ast_distance = kwargs["ast_distance"]
        ast_rpe = kwargs["ast_rpe"]

        batch_size = code_len.size(0)
        # embed and encode the source sequence
        code_rep = self.embedder(
            code_word_rep, code_char_rep, code_type_rep, mode="encoder"
        )
        memory_bank, layer_wise_outputs = self.encoder(
            code_rep, code_len, code_struc_rep, ast_rpe, ast_distance
        )  # B x seq_len x h

        # repo distribution
        repo_dis = self.repo_dis_fc(memory_bank[:, 0])
        mask = sequence_mask(repo_len, max_len=repo_dis.size(1)).to(
            repo_dis.device
        )
        repo_dis.masked_fill_(~mask, -float("inf"))

        repo_logits = torch.softmax(repo_dis, dim=-1)

        del mask
        repo_top_k = repo_logits.topk(self.repo_k, dim=-1)[1]

        reps = torch.zeros(batch_size, self.decoder.input_size).to(
            repo_logits.device
        )

        for j in range(batch_size):
            data = []
            for i in range(self.repo_k):
                # if i>= repo_len[j]:
                #     continue
                tmp = np.load(
                    f"{repo_dir[j]}/{repo_top_k[j][i].item()}.npy"
                )
                data.append(tmp)
            reps[j] = torch.FloatTensor(data).mean(dim=0)

        repo_rep = self.repo_fc(reps)

        # embed and encode the target sequence
        summ_emb = self.embedder(
            summ_word_rep, summ_char_rep, mode="decoder"
        )

        input_proj = self.input_proj(repo_rep).unsqueeze(1)
        summ_emb = summ_emb + input_proj
        summ_emb = self.emb_drop(summ_emb)

        summ_pad_mask = ~sequence_mask(
            summ_len, max_len=summ_emb.size(1)
        )
        enc_outputs = (
            layer_wise_outputs if self.layer_wise_attn else memory_bank
        )
        layer_wise_dec_out, attns = self.decoder(
            enc_outputs,
            code_len,
            summ_pad_mask,
            summ_emb,
        ...
        if self._copy:
            ...
            scores = scores[
                :, :-1, :
            ].contiguous()  # `batch x tgt_len - 1 x vocab_size`
            ml_loss = self.criterion(
                scores.view(-1, scores.size(2)), target.view(-1)
            )
    ...
    def __generate_sequence(
        self, params, choice="greedy", tgt_words=None
    ):
        ...
        if tgt_words is None:
            ...
            tgt_words = tgt_words.expand(batch_size).unsqueeze(
                1
            )  # B x 1

        tgt_chars = None
        if self.embedder.use_tgt_char:
            tgt_chars = params["tgt_dict"].word_to_char_ids(
                constants.BOS_WORD
            )
        ...
        max_mem_len = (
            params["memory_bank"][0].shape[1]
            if isinstance(params["memory_bank"], list)
            else params["memory_bank"].shape[1]
        )

        dec_states = self.decoder.init_decoder(
            params["src_len"], max_mem_len
        )

        attns = {"coverage": None}
        enc_outputs = (
            params["layer_wise_outputs"]
            if self.layer_wise_attn
            else params["memory_bank"]
        )

        repo_rep = params["repo_rep"]
        input_proj = self.input_proj(repo_rep).unsqueeze(1)

        # +1 for <EOS> token
        for idx in range(params["max_len"] + 1):
            tgt = self.embedder(
                tgt_words, tgt_chars, mode="decoder", step=idx
            )

            tgt = tgt + input_proj
            tgt = self.emb_drop(tgt)
            tgt_pad_mask = tgt_words.data.eq(constants.PAD)
            layer_wise_dec_out, attns = self.decoder.decode(
                tgt_pad_mask,
                tgt,
                # repo_rep,
                enc_outputs,
            ...
            if self._copy:
                ...
                if params["src_mask"] is not None:
                    mask = (
                        params["src_mask"].byte().unsqueeze(1)
                    )  # Make it broadcastable.
                    copy_score.data.masked_fill_(mask, -float("inf"))
                attn_copy = f.softmax(copy_score, dim=-1)
                prediction = self.copy_generator(
                    decoder_outputs, attn_copy, params["src_map"]
                )
            ...
            words = self.__tens2sent(
                tgt, params["tgt_dict"], params["source_vocab"]
            )
            tgt_chars = None
            if self.embedder.use_tgt_char:
                tgt_chars = [
                    params["tgt_dict"].word_to_char_ids(w).tolist()
                    for w in words
                ]
    ...
    def decode(
        ...
        **kwargs,
        ...
        ast_rpe = kwargs["ast_rpe"]
        memory_bank, layer_wise_outputs = self.encoder(
            word_rep, code_len, code_struc_rep, ast_rpe, ast_distance
        )  # B x seq_len x h
        ...
        params["repo_rep"] = repo_rep
        params["repo_len"] = repo_len

        dec_preds, attentions, copy_info, _ = self.__generate_sequence(
            params, choice="greedy"
        )
