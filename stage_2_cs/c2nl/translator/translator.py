...
class Translator(object):
    ...
    def translate_batch(self, batch_inputs):
        ...
        repo_dir = batch_inputs["repo_dir"]
        repo_len = batch_inputs["repo_len"]
        code_struc_rep = batch_inputs["code_struc_rep"]
        ast_distance = batch_inputs["ast_distance"]
        ast_rpe = batch_inputs["ast_rpe"]
        ...
        exclusion_tokens = set(
            [self.model.tgt_dict[t] for t in self.ignore_when_blocking]
        )
        ...
        model = (
            ...
            if hasattr(self.model.network, "module")
            else self.model.network
        )
        ...
        copy_generator = (
            model.copy_generator if self.copy_attn else None
        )
        ...
        if model_name == "Transformer":
            memory_bank, layer_wise_outputs = encoder(
                code_rep,
                code_len,
                code_struc_rep,
                ast_rpe,
            ...
            repo_dis = model.repo_dis_fc(memory_bank[:, 0])
            ...
            del mask
            repo_top_k = repo_logits.topk(model.repo_k, dim=-1)[1]
            batch_size = repo_logits.size(0)
            reps = torch.zeros(batch_size, model.decoder.input_size).to(
                repo_logits.device
            )
            for j in range(batch_size):
                data = []
                for i in range(model.repo_k):
                    tmp = np.load(
            ...
            repo_rep = model.repo_fc(reps)
            src_lens = code_len.repeat(beam_size)
            dec_states = decoder.init_decoder(
                src_lens, memory_bank.shape[1]
            )
        else:
        ...
        repo_rep = (
            repo_rep.unsqueeze(1)
            .expand(-1, beam_size, -1)
            .contiguous()
            .view(-1, repo_rep.size(-1))
        )
        input_proj = model.input_proj(repo_rep).unsqueeze(1)
        ...
        for i in range(self.max_length + 1):
            ...
            if self.copy_attn:
                inp = inp.masked_fill(
                    inp.gt(len(self.model.tgt_dict) - 1), constants.UNK
                )

            inp_chars = None
            if embedder.use_tgt_char:
                words = [
                    self.model.tgt_dict[w] for w in inp[:, 0].tolist()
                ]
                inp_chars = [
                    self.model.tgt_dict.word_to_char_ids(w).tolist()
                    for w in words
                ]
            ...
            if model_name == "Transformer":
                ...
                attn["std"] = attn["std"][0]
                if self.copy_attn:
                    _, copy_score, _ = model.copy_attn(
                        dec_out,
                        memory_bank,
                    ...
                    if code_mask_rep is not None:
                        mask = code_mask_rep.byte().unsqueeze(
                            1
                ...
                dec_out, attn = decoder.decode(
                    tgt, dec_states, memory_bank, memory_lengths
                )
                if "std" in attn:
            ...
            if self.copy_attn:
                out = copy_generator.forward(
                    dec_out, attn["copy"], src_map
                )
                out = out.squeeze(1)
                ...
                for b in range(out.size(0)):
                    for bx in range(out.size(1)):
                        if blank[bx]:
                            blank_b = torch.Tensor(blank[bx]).to(
                                code_word_rep
                            )
                            fill_b = torch.Tensor(fill[bx]).to(
                                code_word_rep
                            )
                            out[b, bx].index_add_(
                                0,
            ...
            for j, b in enumerate(beam):
                ...
                if model_name != "Transformer":
                    if isinstance(
                        ...
                        dec_states[0].beam_update(
                            j, b.get_current_origin(), beam_size
                        )
                        dec_states[1].beam_update(
                            j, b.get_current_origin(), beam_size
                        )
                    else:
                        dec_states.beam_update(
                            j, b.get_current_origin(), beam_size
                        )
