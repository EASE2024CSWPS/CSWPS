...
import numpy as np
import torch
from c2nl.eval.bleu import Bleu, corpus_bleu, nltk_corpus_bleu
...
from c2nl.utils.copy_utils import (
    align,
...
def add_test_args(parser):
    ...
    # bsearch.add_argument('--class_repo', type=str, default='',
    #                      help='class repo')
    bsearch.add_argument("--repos", type=str, default="", help="repos")
...
def set_defaults(args):
    ...
    args.log_file = os.path.join(
        args.model_dir, args.model_name + "_beam.txt"
    )
...
def prepare_batch(batch, model):
    ...
    if model.args.copy_attn:
        ...
        source_map = (
            source_map.cuda(non_blocking=True)
            if args.cuda
            else source_map
        )
        alignment = None
        blank, fill = collapse_copy_scores(
            model.tgt_dict, batch["src_vocab"]
        )
    else:
    ...
    # repo_rep = batch['repo_rep']
    repo_len = batch["repo_cnt"]
    code_struc_rep = batch["code_struc_rep"]
    ast_distance = batch["ast_distance"]
    ast_rpe = batch["ast_rpe"]

    repo_dir = []

    for repo in batch["repos"]:
        repo_dir.append(os.path.join(model.args.repos, repo))
    ...
    if args.cuda:
        ...
        if code_struc_rep is not None:
            ...
            ast_distance = ast_distance.cuda(non_blocking=True)
            ast_rpe = ast_rpe.cuda(non_blocking=True)
    ...
    batch_inputs["repo_dir"] = repo_dir
    batch_inputs["repo_len"] = repo_len
    batch_inputs["code_struc_rep"] = code_struc_rep
    batch_inputs["ast_distance"] = ast_distance
    batch_inputs["ast_rpe"] = ast_rpe
    return batch_inputs
...
def validate_official(args, data_loader, model):
    ...
    builder = TranslationBuilder(
        model.tgt_dict, n_best=args.n_best, replace_unk=args.replace_unk
    )
    ...
    with torch.no_grad():
        ...
        for batch_no, ex in enumerate(pbar):
            ...
            translations = builder.from_batch(
                ret, ex["code_tokens"], targets, ex["src_vocab"]
            )

            src_sequences = [code for code in ex["code_text"]]
            for eid, trans, src in zip(
                ids, translations, src_sequences
            ):
...
def main(args):
    ...
    # model.args.class_repo = args.class_repo
