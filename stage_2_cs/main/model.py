...
from c2nl.utils.misc import count_file_lines, tens2sen
...
class Code2NaturalLanguage(object):
    ...
    def __init__(self, args, src_dict, tgt_dict, state_dict=None):
        ...
        if args.model_type == "rnn":
            ...
            raise RuntimeError(
                "Unsupported model: %s" % args.model_type
            )
    ...
    def update(self, ex):
        ...
        if self.args.copy_attn:
            ...
            alignment = (
                alignment.cuda(non_blocking=True)
                if self.use_cuda
                else alignment
            )

            blank, fill = collapse_copy_scores(
                self.tgt_dict, ex["src_vocab"]
            )
        ...
        repo_len = ex["repo_cnt"]
        code_struc_rep = ex["code_struc_rep"]
        ast_distance = ex["ast_distance"]
        ast_rpe = ex["ast_rpe"]
        ...
        if self.use_cuda:
            ...
            repo_len = repo_len.cuda(non_blocking=True)
        ...
        for repo in ex["repos"]:
            repo_dir.append(os.path.join(self.args.repos, repo))

        # Run forward
        net_loss = self.network(
        ...
        loss = (
            net_loss["ml_loss"].mean()
            if self.parallel
            else net_loss["ml_loss"]
        )
        loss_per_token = (
            net_loss["loss_per_token"].mean()
            if self.parallel
    ...
    def predict(self, ex, replace_unk=False):
        ...
        predictions = tens2sen(
            decoder_out["predictions"], self.tgt_dict, ex["src_vocab"]
        )
        if replace_unk:
    ...
    def load(filename, new_args=None):
        ...
        return Code2NaturalLanguage(
            args, src_dict, tgt_dict, state_dict
        )
