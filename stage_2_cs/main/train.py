...
def add_train_args(parser):
    ...
    general.add_argument(
        "--project_name", type=str, default="", help="project name"
    )
    ...
    general.add_argument("--repos", type=str, default="", help="repos")
    general.add_argument(
        "--train_guid",
        nargs="+",
        type=str,
        default=["train.token.guid"],
        help="Preprocessed train guid file",
    )
    general.add_argument(
        "--dev_guid",
        nargs="+",
        type=str,
        default=["valid.token.guid"],
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        if num_dataset > 1:
            ...
            if len(args.train_guid) == 1:
                args.train_guid = args.train_guid * num_dataset
        ...
        for i in range(num_dataset):
            ...
            print(data_dir, args.train_repo)
            train_repo = os.path.join(data_dir, args.train_repo[i])
            train_guid = os.path.join(data_dir, args.train_guid[i])
            ...
            if args.use_code_type:
                train_src_tag = os.path.join(
                    data_dir, args.train_src_tag[i]
                )
            ...
            args.train_guid_files.append(train_guid)
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    logger.info(
        "Num words in source = %d and target = %d"
        % (len(src_dict), len(tgt_dict))
    )

    # Initialize model
    model = Code2NaturalLanguage(
        config.get_model_args(args), src_dict, tgt_dict
    )
...
def train(args, data_loader, model, global_stats, ml_logger=None):
    ...
    # ml_logger.report_scalar(title='train', series='loss', value=ml_loss.avg, iteration=current_epoch)
    # ml_logger.report_scalar(title='train', series='ppl',  value=perplexity.avg,  iteration=current_epoch)
...
def validate_official(
    ...
    examples = 0
    sources, hypotheses, references, copy_dict = (
        dict(),
        dict(),
        dict(),
        dict(),
    ...
    with torch.no_grad():
        ...
        for idx, ex in enumerate(pbar):
            ...
            predictions, targets, copy_info = model.predict(
                ex, replace_unk=True
            )

            src_sequences = [code for code in ex["code_text"]]
            examples += batch_size
            for key, src, pred, tgt in zip(
                ex_ids, src_sequences, predictions, targets
            ):
                hypotheses[key] = [pred]
                references[key] = (
...
def eval_accuracies(
    ...
    rouge_l, ind_rouge = rouge_calculator.compute_score(
    ...
    if mode == "test":
        ...
        meteor, _ = meteor_calculator.compute_score(
            references, hypotheses
        )
    else:
        meteor = 0
...
def main(args, ml_logger):
    ...
    if args.only_test:
        ...
        if args.checkpoint and os.path.isfile(
            args.model_file + ".checkpoint"
        ):
