...
class TranslationBuilder(object):
    ...
    def _build_target_tokens(self, src_vocab, src_raw, pred, attn):
        tokens = []
        for tok in pred:
            tok = tok if isinstance(tok, int) else tok.item()
    ...
    def from_batch(
        self, translation_batch, src_raw, targets, src_vocabs
    ):
        ...
        for b in range(batch_size):
            ...
            pred_sents = [
                self._build_target_tokens(
                    src_vocab, src_raw[b], preds[b][n], attn[b][n]
                )
                for n in range(self.n_best)
            ]
            translation = Translation(
                targets[b], pred_sents, attn[b], pred_score[b]
            )
