    ...
    def advance(self, word_probs, attn_out):
        ...
        * `attn_out`- attention at the last step
        ...
        if len(self.prev_ks) > 0:
            beam_scores = word_probs + self.scores.unsqueeze(
                1
            ).expand_as(word_probs)
        ...
        best_scores, best_scores_id = flat_beam_scores.topk(
        ...
        for i in range(self.next_ys[-1].size(0)):
            if self.next_ys[-1][i] == self._eos:
                global_scores = self.global_scorer.score(
                    self, self.scores
                )
                s = global_scores[i]
...
class GNMTGlobalScorer(object):
    ...
    def score(self, beam, logprobs):
        ...
        if not beam.stepwise_penalty:
            penalty = self.cov_penalty(
                beam.global_state["coverage"], self.beta
            )
    ...
    def update_score(self, beam, attn):
        ...
        if "prev_penalty" in beam.global_state.keys():
            ...
            penalty = self.cov_penalty(
                beam.global_state["coverage"] + attn, self.beta
            )
