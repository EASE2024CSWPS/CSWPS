...
from c2nl.modules.util_class import Elementwise
...
class VecEmbedding(nn.Module):
    def __init__(
        self, vec_size, emb_dim, position_encoding=False, dropout=0
    ):
...
class Embeddings(nn.Module):
    ...
    def __init__(
        ...
        self.embedding_size = (
            sum(emb_dims) if feat_merge == "concat" else word_vec_size
        )
    ...
    def init_word_vectors(self, vocabulary, embeddings_index):
        ...
        pretrained = torch.FloatTensor(
            len(vocabulary), self.word_vec_size
        ).zero_()
