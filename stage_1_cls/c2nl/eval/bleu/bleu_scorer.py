    ...
    def compute_score(self, option=None, verbose=0):
        ...
        for comps in self.ctest:
            ...
            if self.special_reflen is None:  # need computation
                reflen = self._single_reflen(
                    comps["reflen"], option, testlen
                )
            else:
        ...
        if ratio > 1.0:
            ...
            bleus = [bleus[k] * bp for k in range(n)]
        ...
        if min(bleus) > 0:
            p_log_sum = sum((1.0 / n) * math.log(p) for p in bleus)
        ...
        bleu = geo_mean * bp
        self._score = bleus
        return self._score, bleu_list, bleu
