...
class Vocabulary(object):
    def __init__(self, no_special_token=False):
        if no_special_token:
            self.tok2ind = {PAD_WORD: PAD, UNK_WORD: UNK}
            self.ind2tok = {PAD: PAD_WORD, UNK: UNK_WORD}
        else:
            self.tok2ind = {
                PAD_WORD: PAD,
                UNK_WORD: UNK,
    ...
    def __getitem__(self, key):
        if type(key) == int:
            ...
            return self.tok2ind.get(
                self.normalize(key), self.tok2ind.get(UNK_WORD)
            )
        else:
...
class UnicodeCharsVocabulary(Vocabulary):
    ...
    def __init__(self, words, max_word_length, no_special_token):
