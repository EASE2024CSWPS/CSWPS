...
from c2nl.inputters.constants import (
    AST_TYPE_MAP,
    BOS_WORD,
    DATA_LANG_MAP,
    EOS_WORD,
...
from c2nl.inputters.vocabulary import UnicodeCharsVocabulary, Vocabulary
...
def process_examples(
    ...
    TAG_TYPE_MAP = (
        TOKEN_TYPE_MAP if code_tag_type == "subtoken" else AST_TYPE_MAP
    )
    code = Code()
    code.text = source
...
def load_data(
    ...
    with open(filenames["src"]) as f:
        sources = [
            line.strip()
            for line in tqdm(
                f, total=count_file_lines(filenames["src"])
            )
        ]
    ...
    if filenames["tgt"] is not None:
        with open(filenames["tgt"]) as f:
            targets = [
                line.strip()
                for line in tqdm(
                    f, total=count_file_lines(filenames["tgt"])
                )
            ]
    else:
    ...
    if filenames["src_tag"] is not None:
        with open(filenames["src_tag"]) as f:
            source_tags = [
                line.strip()
                for line in tqdm(
                    f, total=count_file_lines(filenames["src_tag"])
                )
            ]
    else:
        source_tags = [None] * len(sources)

    if filenames["repo"] is not None:
        with open(filenames["repo"]) as f:
            repos = [
                line.strip()
                for line in tqdm(
                    f, total=count_file_lines(filenames["repo"])
                )
            ]
    else:
        repos = [None] * len(sources)

    assert len(sources) == len(source_tags) == len(targets)

    examples = []
    for repo, src, src_tag, tgt in tqdm(
        zip(repos, sources, source_tags, targets), total=len(sources)
    ):
        if dataset_name in ["java", "python", "mp", "cp"]:
            _ex = process_examples(
                LANG_ID_MAP[DATA_LANG_MAP[dataset_name]],
                repo,
                src,
                src_tag,
                tgt,
...
def process_summary(lang_id, repo, source, max_src_len, uncase=False):
    summ = source.lower() if uncase else source
    summ_tokens = summ.split()
    summ_tokens = summ_tokens[:max_src_len]
    if len(summ_tokens) == 0:
        return None
...
def load_summary(
    ...
    assert len(sources) == len(repos)

    examples = []
    for repo, src in tqdm(zip(repos, sources), total=len(sources)):
        if dataset_name in ["java", "python", "CodeXGLUE", "PCS"]:
            _ex = process_summary(
                LANG_ID_MAP[DATA_LANG_MAP[dataset_name]],
                repo,
                src,
...
def process_ae(lang_id, repo, source, max_src_len, uncase=False):
    summ = source.lower() if uncase else source
    summ_tokens = summ.split()
    summ_tokens = summ_tokens[:max_src_len]
    if len(summ_tokens) == 0:
    ...
    src = Summary()
    src_tokens = deepcopy(summ_tokens)
    src.text = " ".join(src_tokens)
...
def load_words(args, examples, fields, dict_size=None):
    ...
    dict_size = (
        dict_size - 2 if dict_size and dict_size > 2 else dict_size
    )
...
def build_word_dict(
    args, examples, fields, dict_size=None, no_special_token=False
):
...
def build_word_and_char_dict(
    ...
    dictioanry = UnicodeCharsVocabulary(
        words, args.max_characters_per_token, no_special_token
    )
    return dictioanry
